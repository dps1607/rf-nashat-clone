...
if [ -d "$CHROMA_DB_PATH" ] && [ -n "$(find "$CHROMA_DB_PATH" -mindepth 1 -type d 2>/dev/null)" ]; then
    echo "[bootstrap] chroma_db already populated at $CHROMA_DB_PATH (has collection subdirs), leaving alone"
    ...
    if curl -fsSL --retry 3 --retry-delay 5 -o "$TMPTAR" "$CHROMA_BOOTSTRAP_URL"; then
        ...
        echo "[bootstrap] clearing skeleton chroma_db before extract..."
